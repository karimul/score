...
class NCSNv2(nn.Module):
    def __init__(self, config):
        ...
        self.begin_conv = nn.Conv2d(
            config.data.channels, nf, 3, stride=1, padding=1)

        self.normalizer = self.norm(nf, config.model.num_scales)
        self.end_conv = nn.Conv2d(
            nf, config.data.channels, 3, stride=1, padding=1)
        ...
        self.refine1 = RefineBlock(
            [2 * self.nf], 2 * self.nf, act=act, start=True)
        self.refine2 = RefineBlock(
            [2 * self.nf, 2 * self.nf], 2 * self.nf, act=act)
        self.refine3 = RefineBlock(
            [2 * self.nf, 2 * self.nf], self.nf, act=act)
        self.refine4 = RefineBlock(
            [self.nf, self.nf], self.nf, act=act, end=True)
    ...
    def forward(self, x, y):
        ...
        used_sigmas = self.sigmas[y].view(
...
class NCSNv2_128(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.refine1 = RefineBlock(
            [4 * self.nf], 4 * self.nf, act=act, start=True)
        self.refine2 = RefineBlock(
            [4 * self.nf, 4 * self.nf], 2 * self.nf, act=act)
        self.refine3 = RefineBlock(
            [2 * self.nf, 2 * self.nf], 2 * self.nf, act=act)
        self.refine4 = RefineBlock(
            [2 * self.nf, 2 * self.nf], self.nf, act=act)
        self.refine5 = RefineBlock(
            [self.nf, self.nf], self.nf, act=act, end=True)
